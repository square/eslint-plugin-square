    ...
    return {
      CallExpression(node) {
        const serviceName =
            ...
            context.options[0].serviceName || DEFAULT_SERVICE_NAME;
        const enforceStringLiteral =
            context.options &&
            context.options.length > 0 &&
            context.options[0].enforceStringLiteralKeys || false;

        if (
            (isT(node) || isIntlT(node, serviceName) || isThisIntlT(node, serviceName)) &&
            (node.arguments.length === 1 || node.arguments.length === 2)
        ) {
          if ((enforceStringLiteral && node.arguments[0].type !== 'Literal') ||
